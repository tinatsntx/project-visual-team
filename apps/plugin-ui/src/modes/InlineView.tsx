 ...
 */
export function InlineView({ task, recentEvents }: { task: TaskSnapshot; recentEvents: VisualEvent[] }) {
  ...
  return (
    <section className="vt-inline" aria-label={`Visual team status: ${task.title}`}>
      <div className="vt-inline-main">
        {lead && (
          <RobotAvatar role={lead.role} state={lead.state} label={lead.label} animated={!reduced} />
        )}
      ...
      {support.length > 0 && (
        <ul className="vt-support" aria-label="Supporting team members">
          {support.map((w) => (
            <li key={w.id}>
              <RobotAvatar role={w.role} state={w.state} label={w.label} size={24} animated={!reduced} />
